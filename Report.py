...
import pandas as pd
from utils import sentiment_from_rating, sentiment_from_text, detect_anomaly, detect_buzzer

st.set_page_config(page_title="Report", page_icon="📄", layout="wide")

st.title("Report")
st.write("Halaman ini menampilkan ringkasan hasil analisis sentimen dan deteksi buzzer.")
...
if uploaded_file is not None:
    ...
    if "buzzer_status" not in df.columns:
        df["buzzer_status"] = df.apply(
            lambda row: detect_buzzer(row["review"], row["rating"]), axis=1
        )
else:
...
buzzer = (df["buzzer_status"] == "buzzer").sum()
non_buzzer = (df["buzzer_status"] == "non_buzzer").sum()
...
c5.metric("Buzzer", buzzer)
c6.metric("Non-Buzzer", non_buzzer)
...
# Grafik buzzer
st.subheader("Distribusi Status Buzzer")
st.bar_chart(df["buzzer_status"].value_counts())

# Insight sederhana
st.subheader("Insight Penelitian")

persen_buzzer = round((buzzer / total_data) * 100, 2) if total_data > 0 else 0
...
st.write(f"3. Review yang terindikasi buzzer sebanyak **{buzzer}** data atau sekitar **{persen_buzzer}%** dari total data.")
st.write("4. Indikasi buzzer ditentukan berdasarkan ketidaksesuaian antara isi komentar dan rating bintang.")
st.write("5. Contoh: komentar positif tetapi rating 1 atau 2, atau komentar negatif tetapi rating 4 atau 5.")

# Contoh data buzzer
st.subheader("Contoh Review yang Terindikasi Buzzer")

df_buzzer = df[df["buzzer_status"] == "buzzer"].copy()
...
if len(df_buzzer) > 0:
    show_cols = [c for c in [
        ...
        "anomaly_status",
        "buzzer_status"
    ] if c in df_buzzer.columns]

    st.dataframe(
        df_buzzer[show_cols].head(20),
        use_container_width=True,
        hide_index=True
    )
else:
    st.info("Belum ada data yang terindikasi buzzer.")