...
import pandas as pd
from utils import load_data, sentiment_from_text, sentiment_from_rating, detect_anomaly, detect_buzzer
...
with tab1:
    ...
    if st.button("Analisis Komentar"):
        if not comment.strip():
            ...
            if buzzer == "buzzer":
                st.error("Komentar ini terindikasi buzzer karena isi komentar tidak sesuai dengan rating.")
            else:
                st.success("Komentar ini tidak terindikasi buzzer.")
    ...
    if uploaded_file is not None:
        try:
            ...
            if "review" not in df_upload.columns or "rating" not in df_upload.columns:
                ...
                df_upload["buzzer_status"] = df_upload.apply(
